...
*Which of the three axes [B] offers was OQ-4. **Resolved at M5: initial positions and velocities**, as one declared scalar applied to named entities before `start`. Decided by measurement rather than by argument — `docs/m5-oq4.md`, 18 probe runs — on fidelity, on what CR-AS-4 can soundly assert, and on ordering, which is what a **trend** requires and a scenario name cannot have. R9's premise, that the choice might be forced by the read-only install tree, did not hold for any of the three and was closed at M2.*
...
>
> CR-DET-1's `samples_not_recorded` exclusion had never fired in any run this project had made; M4 made it fire on a real capture (`--queue-size 4`, 2 755 samples not recorded) rather than shipping it untested. Detail in `docs/m4-determinism.md`.

### M5 — One parameterisation axis, and a sweep (est. 1.5 days)
[B] step 5. **OQ-4 decided by measurement** (`docs/m5-oq4.md`) — initial positions and velocities, as one declared scalar. Declared in a campaign configuration file, swept across a campaign, and presented so the trend is visible in the report's own format.
...
>   original refusal is kept in `docs/m7-evidence.md` §4, because it is what makes the pass mean
>   anything.
