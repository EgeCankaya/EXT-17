...
| Sweep legibility | n/a | A result that varies with the parameter, presented so the trend is visible without further processing | Mentor review of the sweep report | M5 - **split at M7, and both halves reported.** **[B]'s acceptance criterion 3 is MET on measured evidence**: it reads *"a parameter sweep shows a result that varies with the parameter, presented so the trend is visible"*, it **names no reviewer and no method**, and the committed twenty-run sweep shows engagement rising 47 -> 65 to a peak at 170-190 m/s and falling to 54 at 380, three conditions flipping at three thresholds, and the run outcome flipping with them. **The mentor-review verification method was NOT executed**, by DRI decision on 2026-09-01, schedule being the binding constraint. **That method is this project's own invention, not the client's**, and it is recorded as not executed rather than rewritten to one this project already satisfies - swapping a verification method for one you pass is what makes a metrics table worthless. A mentor review remains available and would add confidence; nothing in [B] requires it |
...
| **OQ-3** | Is `n8ro-sim-app.exe --sim-config SimEngineHost_SharedMemory --model-path <dir> --schema-file <name>` the intended production invocation? | **Decided at M7 - adopted. The mentor confirmation [B] asks for did NOT happen.** | Was: M2 | **DECIDED by the DRI on 2026-09-01**, `docs/m7-oq2-oq3.md` s2, part by part. (a) The bus route is [B]'s own **first-listed** surface, and the scripting namespaces are explicitly conditional - *"if you drive it that way"*. (b) `N8RO_RELEASE` is **required**, because without it the binary does not do what [B]'s own description of it says it does. (c) `SimEngineHost_SharedMemory` over any best-effort variant, on [B]'s *"campaign runs are for the closed configuration"* and its rule 4, *"determinism first"*. (d) The degraded terrain configuration stays, because provisioning it now would split a comparable body of evidence in two at the last milestone. (e) A console control event ends the host, which [B]'s rule 1 requires in substance - *"not a still-running host"* - and the non-zero exit is normal here and is not read as a crash. (f) `C:\N8RO\bin` on `PATH` is a second, separate precondition, verified again at M7. **No code changed.** **What [B] literally asks - *"confirm the invocation with your mentor"* - did not happen**, cannot be discharged by reading [B], and is now a named limit in the README rather than a queue item |
...
> **M7 cannot fully close, and that is a decision rather than a slip.** Its validation line says
> *"CR-DOC-1, CR-DOC-2, and every success metric"*, and **two of those cannot be met by this
> project alone**:
>
> - **"Sweep legibility"** names *mentor review of the sweep report* as its measurement method.
>   No mentor has reviewed it. **M7 reports this metric as UNMET rather than claiming it** — a
>   metric whose named method did not happen is not a metric that passed, and substituting a
>   different method would be re-writing the target to fit the result.
> - **The 5-minute recording** needs a person (R10). M7 delivers the script it would follow — so
>   the remaining work is the recording and not the preparation — and reports the deliverable as
>   outstanding rather than substituting a written walkthrough and calling it met. That
>   substitution is precisely the failure R10 exists to name.
>
> Both trace to **E-1 being deferred by DRI decision** (`docs/findings.md` §E), which recorded
> this cost in advance. It is carried forward unchanged rather than rediscovered.
