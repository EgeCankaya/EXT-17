...
**Date:** 2026-08-31
**Revision:** 2 — upstream delivered its byte-limited capture, which retires R3's premise and reshapes CR-CAP-5. No requirement removed; one strengthened. See §"Revision history".
**Status:** Draft
...
| Peak campaign disk footprint | n/a | Bounded and stated in the README, at two levels: **per capture** by the upstream recorder's `--capture-max-bytes` (EXT-08 producer 0.9.0), and **per campaign** by CR-CAP-5. ~64 MB per 200 s run [C2], ~465 B/record [C3] | Measured across the 20-run campaign | M3 |
...
| A scenario with a varyable axis that does not require writing into `C:\N8RO` (OQ-4) | Mentor / scenario catalogue | **Not yet identified** | Blocks M5. See R9 |
...
| **R9 — The parameterisation axis may require writing into a read-only tree.** [B] offers three axes: initial positions and velocities, which entities are present, which scenario | Medium — it could invalidate the chosen axis at M5, mid-project | Unknown — **not yet investigated**, and this document does not guess | **OQ-4**, decision target M5, with a spike at M2 to establish which of [B]'s three axes is varyable without authoring into `C:\N8RO`. "Which scenario from the catalogue" is the fallback, since it requires no authoring at all |
...
| **OQ-4** | Which of [B]'s three parameterisation axes does v1 use? | Open | M5, with a feasibility spike at M2 | [B] offers initial positions and velocities, which entities are present, or which scenario from the catalogue — and settles the count (*"One axis done properly"*) without settling the choice. The deciding constraint is R9: `C:\N8RO` is read-only for this project, so an axis requiring scenario authoring in place is disqualified unless a scratch model database is available. "Which scenario from the catalogue" is the fallback because it needs no authoring. Resolved by the M2 spike |
...
| **OQ-6** | What is the campaign's disk ceiling, what does it do on reaching it, and which per-run bound do we configure upstream? | Open | M3 | Depends on a measurement this project has not yet taken: actual capture size for the chosen scenario at the chosen stop predicate (OQ-1). The inputs are known — ~64 MB per 200 s run [C2], ~465 B/record [C3] — but neither is this project's own measurement of this campaign's runs. **Rev 2 adds a second decision**, now that upstream has one to configure: per run, `--on-size-limit stop` bounds each capture and loses the tail of any run that overruns, while `rotate` keeps the tail and makes a run's capture a multi-file set the reader must stitch (§6.7, and segment ordinals restart per part). **`stop` is the leading candidate** — a campaign that needs the tail of a run has mis-set its stop predicate (OQ-1), and one file per run keeps CR-CAP-4's segment handling simple — but it is not a decision until M3 measures what a run actually costs. Resolved by measuring at M3 and fixing all three in CR-CAP-5 and the README |
...
**Validation:** CR-CAP-1 through CR-CAP-4. The reader parses the vendored fixture, agrees with its trailer, and survives four deliberate mutations. `attached_mid_run: false` and zero orphans on a real run. **Gate: no EXT-08 source has been read, and no EXT-08 identifier appears in EXT-17's source.**
...
- **The parameterisation axis is unchosen (OQ-4) and may be constrained by a read-only install tree (R9).** No axis has been shown feasible yet. *Suggestion: the M2 spike should establish feasibility for at least two of [B]'s three axes so the choice at M5 is not forced.*
...
- **CR-CAP-5's ceiling is unset (OQ-6)**, for the same reason: it depends on a size this project has not yet measured for its own runs.
