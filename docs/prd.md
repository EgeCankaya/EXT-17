...
**Date:** 2026-09-02
**Revision:** 13 — **A documentation cross-check found one deviation this document announced everywhere except here.** [B]'s deliverable 3 asks for a real campaign *"its configuration, **its captured runs** and its report"*; the captures are **not** committed and a `MANIFEST.md` stands in for them. The README has said so since M7 and the rev-7 revision history recorded it — but **CR-DOC-2 still quoted [B]'s clause verbatim with no deviation banner, its acceptance criteria did not mention the substitution, and Appendix A's coverage read 52 fully covered / 1 with a named deviation.** It is now **51 / 2**, CR-DOC-2 carries the banner and the criterion, and Appendix A's Deliverables row names it. **Two supporting defects were fixed with it**: the manifest generator had only ever been run over M6's campaigns, so thirteen earlier ones had nothing standing in for their captures at all — it has now been run backwards over every campaign (fourteen manifests, 94 captures) — and UAC-CR-DOC-2 asked a reviewer to reproduce the committed campaign without saying that reproducing it **into** its own directory overwrites it. **And `contract/` is re-pinned at the SIXTH pin**, EXT-08 `7fe7dec`: the same pass found two links in `condition-file-schema.md` that resolve from nowhere the file lives — including the one citing `src/Geodesy.h`, which is the sentence E-5 was raised to obtain — raised as **E-10**, fixed upstream the same day, vendored back in the same breath (F-38). All four artifacts verified byte-identical to EXT-08 `main`; the full suite re-run, 527 mandatory checks, 0 failures. **No requirement was weakened, no code changed and no quotation changed.** What remains is OQ-2, never answered by [B]'s author. See §"Revision history".
**Revision 12 (superseded):** — **The last outstanding deliverable is DELIVERED: the 5-minute recording was shot and published on 2026-09-01.** [One take, 4 min 10 s](https://drive.google.com/drive/folders/16cR82ynxrcmrzJofwHKdpReNlPj1C--M?usp=sharing), launched, watched and read in that order, shot to a prepared runbook with no step skipped. **CR-DOC-2 is now met in all four of its parts and R10 closes.** The recording is **4:10 against [B]'s "5-minute"**, which is stated rather than rounded up. It carries **no narration and shows nothing from an editor** — the recording is the terminal, which is only possible because the tools caption themselves. The campaign it launches on camera is committed at `campaigns/demo/`, so every number on screen traces to a run record rather than to a video frame. **No code changed, no requirement was removed and no quotation changed.** What remains is OQ-2, never answered by [B]'s author. See §"Revision history".
**Revision 11 (superseded):** — **OQ-3 is ANSWERED IN FULL and E-1 is CLOSED; OQ-2 is CONCURRED with and still not answered.** The mentor confirmed the last two parts of OQ-3 — **(a) yes**, the bus-publish route is the intended control path, and **(c) *"pick the one you prefer"***, so no host variant is prescribed and `SimEngineHost_SharedMemory` stays as this project's own explicitly-delegated choice. [B]'s *"confirm the invocation with your mentor"* is **discharged**. The mentor also **concurred** with the content gate basis, independently — which is a second opinion and **not** a ruling: criterion 2 is [B]'s author's to discharge and they have still never replied, so **E-2 stays open**. A fifth escalation state, `concurred`, keeps that apart from `answered`. **No behaviour changed**; the reports gained the concurrence and two paired tests that stop it standing in for a ruling. One cosmetic defect fixed: `n8ro-campaign --help` printed `100%%`. What remains: the 5-minute recording, and OQ-2. See §"Revision history".
**Revision 10 (superseded):** — **All four upstream issues came back FIXED, and `contract/` is re-pinned at the fourth pin.** E-3, E-4, E-5 and E-6 were closed by EXT-08 on 2026-09-01 and the corrections are vendored at `ca5118c`. **Nothing this project ships changed** — every correction confirmed a behaviour already implemented and stated beside the text it disagreed with, which is what raising them rather than working around them bought. `condition-file-schema.md` is now vendored **by identity**, which closes **F-19**; F-18's stale pin string closes; and a new finding, **F-38**, records the shape nobody was watching for: *a fixed escalation makes `contract/` stale and nothing notices*. What remains: the 5-minute recording, OQ-2 (never answered by [B]'s author), and OQ-3 (a) and (c). No requirement removed. See §"Revision history".
**Revision 9 (superseded):** — **The mentor replied: four of OQ-3's six parts are now ANSWERED rather than decided, and the sweep-legibility metric is MET by its own named method.** (a) and (c) were not covered and are **not** recorded as answered. A new escalation, **E-6** — the mentor's confirmation that `N8RO_RELEASE` is required makes `PROVENANCE.md` finding 6 demonstrably wrong rather than suspected. **No code changed by any of it.** Rev 8 decided OQ-2 and OQ-3; rev 7 delivered M7. No requirement removed.
**Status:** Draft
...
| Rev | Date | Change |
|----:|------|--------|
| 13 | 2026-09-02 | **A documentation cross-check before handover, and the one finding that matters is a deviation this document was the last place to learn about.** **(a) Deliverable 3's second clause was covered by a substitution nobody had registered here.** [B] asks for *"its captured runs"*; they are not committed, a `MANIFEST.md` stands in, the README announces it in a table cell and the rev-7 history recorded it — and CR-DOC-2 quoted [B]'s clause verbatim with no banner, no acceptance criterion and no Appendix A row, while the headline coverage read **52 fully covered / 1 with a named deviation**. Compare the treatment CR-DET-1's deviation gets: a banner, an ADR, an open question and its own row. **Corrected to 51 / 2**, with the banner, the criterion and the row. **The failure shape is worth naming**: a deviation disclosed in the README and absent from the binding contract is undetectable from either document alone, and neither the milestone closes nor the delivery audit read the two side by side. **(b) The stand-in did not cover what it claimed to.** *"Each campaign carries a `MANIFEST.md` instead"* was true of six files over three campaigns; the generator was written at M6 and never run backwards. It has been run over every campaign that has one — **fourteen manifests, 94 captures, 100% of the uncommitted captures outside the five spike directories**, which are not campaigns and now say so. `campaigns/demo/`, the campaign the recording launches on camera, was the sharpest case: named in the evidence table as tracing the take's numbers to run records, with nothing standing in for its captures. **(c) The README's own commands destroyed the deliverable they documented.** The Quickstart and two of the three *"Configuring a campaign"* invocations passed `--out-dir campaigns\demo` and `--out-dir campaigns\m6-campaign` — both committed. `campaign.log` opens `"wb"`, `campaign.json` is rewritten, runs land in `runs/000` upward with no emptiness check: a reviewer following the README to reproduce the deliverable overwrites it, on a machine whose publication schedule differs by design, so the original survives only in git. Every writing command now names a scratch directory; the read-only `report`, `n8ro-judge` and `n8ro-compare` invocations still name the committed campaign, which is correct. UAC-CR-DOC-2 says it too. **(d) Five documentation defects fixed with them**, none of which changes a requirement: the escalations ledger's title still said *questions out* after the fifth pin made it bidirectional; its status census accounted for six of nine rows; it said *"Four states, not two"* and then defined six; R7 — a defect in **this project's own brief** — had no row in the ledger whose contract is one row per question that leaves this project, and now has one naming EXT-08's E-2 as the vehicle and *sent, no reply* as the status; and *"three weeks"* appeared in three places here, and two more in EXT-08, describing a drift both repositories' logs measure as **one day** — the number appears to have come from [B]'s *"Effort 2–3 weeks"* rather than from the log, and inflating an exposure twentyfold is the one way to make a reader stop trusting the finding it supports. **(e) The sixth `contract/` pin, and a THIRD cause of staleness.** `condition-file-schema.md` — the artifact E-5 created and F-19 closed by making it vendorable **by identity** — carried two links written relative to EXT-08's root `README.md`, where the sections are maintained. Upstream the file is published under `docs/`; here it is vendored into a repository that has no `src/Geodesy.h` at any path. So the sentence *"the formulae are in `src/Geodesy.h` with the constants spelled out, so a third party can reproduce any verdict with a calculator"* — the exact thing E-5 was raised to obtain — pointed at nothing, in both copies. Raised as **E-10**, fixed at EXT-08 `7fe7dec` as absolute URLs (the only form correct in all three places the text lives), and re-pinned in the same breath per F-38. Pin check: all four artifacts byte-identical to EXT-08 `main`. Suite re-run: 527 mandatory checks, 0 failures. **The first three pins moved because the producer grew, the fourth and fifth because an escalation was resolved, and this one because the vendored file was wrong on its own terms** — a third cause, and one a byte check structurally cannot find, because a by-identity vendor copies a broken relative path faithfully and both copies are then identically broken. Only following the link from the copy finds it. **(f) No code changed, no requirement was removed, no quotation changed and no provenance marker relaxed.** |
| 12 | 2026-09-01 | **The 5-minute recording was made, and this revision exists because the difference between *scripted* and *shot* is the whole of R10.** **(a) CR-DOC-2 is met in all four parts.** [One take, 4 min 10 s, published](https://drive.google.com/drive/folders/16cR82ynxrcmrzJofwHKdpReNlPj1C--M?usp=sharing): the condition file validated and then refused on a one-character typo, the campaign launched with one command, ninety seconds of it running unattended, the committed twenty-run report read, the stored runs re-judged and byte-verified, one capture's verdicts including the indeterminate one, and the run-to-run diff naming the first divergence. All three clauses [B] names — *launch a campaign, watch it run, read the report* — in that order. **(b) R10 closes, and closes on its own terms.** The risk was that an acceptance item needing a person would be quietly substituted for; from rev 7 until the film existed, every document here read **outstanding** and the runbook's first line read `NOT RECORDED`. Nothing was ever claimed on the strength of the script, which is the only reason *delivered* means anything now. **(c) It runs 4:10 against "5-minute", and that is stated rather than rounded up.** With no narration the commands take less time than they did when the script had someone talking over them. **(d) The recording carries no narration and shows nothing from an editor** — it is the terminal and nothing else. That is possible only because the tools caption themselves: the gate prints its own basis and the OQ-2 wording, the report prints why its bar is not scaled from zero, and every verdict that is not satisfied prints its reason. A claim that has to be spoken to be present is one a recording cannot evidence, so the script was rewritten as a command runbook before the shoot and is kept as the record of what was filmed. **(e) The evidence is committed even though the film is not.** At 110 MB the take is published beside the repository rather than in it; what is committed is `campaigns/demo/` — the three-run, 200-frame campaign the take launches on camera, with a host-start failure injected into run 1. Gate passed on content, 8503 of 8503 comparable samples agreeing at 99.6835% coverage; outcomes 0 pass, 2 fail, 1 infrastructure error, summing to 3 and merging nothing. So every number visible on screen traces to a record here. **(f) F-36 closes on both halves**, neither by relaxing its terms: the sweep-legibility metric by its own named method at rev 9, and the recording by being made. **(g) Nothing else changed.** No code, no requirement removed, no quotation changed, no provenance marker relaxed. **What remains is OQ-2** — decided by the DRI, concurred with by the mentor, and still never answered by [B]'s author, whose criterion it is. |
| 11 | 2026-09-01 | **Both remaining questions were put to their recipients, and they came back differently — which is the whole reason this document keeps five words for "asked" apart.** **(a) OQ-3 is ANSWERED IN FULL; E-1 CLOSES.** The mentor confirmed the two parts the first relay did not cover: **(a) yes**, the bus-publish route is the intended control path — the reading rev 8 took from [B]'s own ordering; and **(c) "pick the one you prefer"**, which settles that **no `SimEngineHost_*` variant is prescribed**. `SimEngineHost_SharedMemory` therefore stays on the brief's own reasoning — a best-effort transport is one permitted to drop, against *"campaign runs are for the closed configuration"* and *"determinism first"* — and it stays as **this project's own choice, explicitly delegated rather than inferred**. What the answer removes is the risk of having chosen against an intent that existed and had not been written down. **All six parts matched what was already built, so nothing changed**, and [B]'s *"confirm the invocation with your mentor"* is now **discharged** rather than carried as a standing limit. **(b) OQ-2 is CONCURRED with and is STILL NOT ANSWERED, and the row stays open.** The mentor, asked separately and without having seen the reading, also said content. That is a second person entitled to an opinion reaching the same answer independently, and it is worth recording — but OQ-2 asks whether **[B]'s acceptance criterion 2 is discharged**, [B]'s acceptance criteria belong to [B]'s author, and they have still never replied. **A concurrence does not transfer that authority.** **(c) A fifth escalation state, `concurred`**, added to `escalations.md`: it sits *beside* `decided`, not above it, and one row carries it. **(d) The reports carry it, guarded.** `self-test.json` gains `gate.oq2_concurred_by` as a **separate key** beside `gate.oq2_decided_by` and `gate.oq2_answered_by_brief_author` (still `false`), so a consumer cannot compute the last from the others; the campaign log and `--help` say all three things in one breath. **Two paired checks in `tests/determinism_test.cpp`** assert the concurrence may only ever appear in a report that also states [B]'s author has not replied — the failure being guarded is not the wording but somebody later reading "the mentor said content" and deleting the caveat. **(e) No behaviour changed anywhere.** Content was already the default and still is; both comparisons still always run and are always reported. **(f) One cosmetic defect found and fixed while regenerating a golden file: `n8ro-campaign --help` printed `100%%`** — a `printf` escape left in a string emitted with `std::puts`. Recorded as **F-39**; `n8ro-compare` uses `printf` and was correct. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 10 | 2026-09-01 | **All four `contract/` escalations came back FIXED, and this is the first revision written after a defect completed the full circuit — found here, raised there, corrected there, vendored back.** **(a) E-3, E-4, E-5 and E-6 are closed** ([EXT-08 issues #1–#4](https://github.com/EgeCankaya/EXT-08/issues)). §6.7 rule 2 and §11 now name the four counters that sum and say to subtract one per cut. §5.1 states what its frozen-clock test actually establishes and lists **three** shapes — EXT-08 folded in the third, R14's, which was measured after the issue was filed and was never added to it — and §14 now says an emptied self-test is a **refusal rather than a pass**, with retrying named as the wrong fix, so **R12 and R14 are upstream text rather than local discipline**. EXT-08's README R8 block carries both environment preconditions and both failure modes. **(b) E-5 was fixed by a different fix than the one asked for, and the better one**: rather than replying "vendor those two sections too", EXT-08 created `docs/condition-file-schema.md` carrying all four sections, with a test that fails if it drifts from its README — so the digest is **vendored by identity** and **F-19 closes outright**, along with the pin-check judgement call it forced. **(c) `contract/` is re-pinned to EXT-08 `ca5118c`, the fourth pin**, and **F-18's stale pin string closes** with it. **(d) Nothing this project ships changed.** All four corrections confirmed behaviour already built, measured and stated beside the text it disagreed with; `src/assert/Geodesy.h` still states its own constants and now agrees with a vendored sentence rather than with an inference. The full test suite and `n8ro-capture read` on the vendored sample both pass unchanged against the re-pinned files — which is the whole verification a documentation-only pin needs, and is exactly what EXT-08's "clarifications, not a version bump" claim had to be checked against. **(e) One new finding, F-38, and it is a process one: a fixed escalation makes `contract/` stale and nothing notices.** The three earlier drifts were the producer growing and were caught by re-reading `PROVENANCE.md`; this one was caused by this project's own success and has no trigger at all. The rule is now written into `PROVENANCE.md` — a `fixed` row is not finished until the pin has moved and the suite has re-run — and it is recorded as a written rule and **not** a check. **(f) A fifth escalation state, `fixed`**, added to `escalations.md`: stronger than `answered`, because a reply is a sentence and a fix is a diff. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 9 | 2026-09-01 | **The mentor replied, and four things stop being decisions and become answers.** Relayed by the DRI on 2026-09-01. **(a) OQ-3's parts (b), (d), (e) and (f) are ANSWERED**: `N8RO_RELEASE` **is** expected to be set in production; the degraded terrain configuration **is** expected and should be left as it is; a console control event **is** the intended way to end an unattended host **and** its non-zero exit (`0xC000013A`) **is** expected; and `C:\N8RO\bin` on `PATH` **is** a known second precondition. Every one of them confirms what was already built and measured, so **no code changed**. **(b) Parts (a) and (c) were NOT covered in what was relayed and are NOT recorded as answered.** They stay `decided` by the DRI on a reading of [B]'s own words. Recording an answer nobody gave is the one failure this project's four-state escalation vocabulary exists to prevent, and the same rule applies to a mentor as to [B]'s author. **(c) The sweep-legibility success metric is MET, by its own named method** — the mentor reviewed the sweep report and confirmed it reads. It was reported **unmet** at revs 7 and 8 on the grounds that the artifact exceeding the target was not a substitute for the named method; the method has now been executed, so the row is met on the terms it was written on rather than on relaxed ones. **(d) A new escalation, E-6**: the mentor's confirmation of (b) turns F-17 from suspected to demonstrated — `PROVENANCE.md` finding 6 documents the headless invocation **without** `N8RO_RELEASE`, and following it exactly produces a host that refuses every 42-entity scenario load while sitting idle rather than failing. Raised with EXT-08. **(e) What remains outstanding is one deliverable and three upstream replies**: the 5-minute recording (needs a person), OQ-2 (still never answered by [B]'s author, still `decided`), and E-3, E-4, E-5, E-6 with EXT-08. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 8 | 2026-09-01 | **OQ-2 and OQ-3 are DECIDED by the DRI, and neither was ANSWERED — and this revision exists mostly to keep those two words apart.** Schedule became the binding constraint; the DRI authorised the implementer to decide both from [B]'s own words rather than wait for recipients who had not replied in five milestones. **(a) OQ-2: content.** The deciding passage is not criterion 2's *"identical captures"* — it is [B]'s statement of what the self-test is **for**: *"if it ever fails, you have found either a defect in your harness or something far more interesting, and you must be able to tell which."* A byte gate fails **190 of 190** on this platform, identically whether the harness is clean or broken, so it distinguishes neither case and defeats the stated purpose of the very test criterion 2 is about. The content gate passed 190 of 190 clean **and failed on a real pair for a real reason** at M6, which is [B]'s harness-defect case caught and named. Four further passages read for content, and *"byte"* and *"hash"* appear nowhere in [B]. **(b) OQ-3: adopted**, its six parts answered from [B] where [B] speaks — the bus route is [B]'s first-listed surface and scripting is explicitly conditional; `SimEngineHost_SharedMemory` follows from *"campaign runs are for the closed configuration"* and *"determinism first"* — and from measurement where it does not. **(c) NO CODE CHANGED by either.** `content` was already the default and the invocation is the one measured across ~100 runs since M1. What moved is what this project is entitled to *say*: the criterion-2 deviation goes from **named-and-unruled** to **named-and-ruled-by-the-DRI**, and criterion 2 is now claimed as met **under the content reading**, said wherever it is claimed and never claimed under a byte reading. **(d) A fourth escalation state, `decided`**, distinct from `answered`: *a person entitled to decide it did, on stated evidence, and the original recipient still has not spoken*. Every artifact says both halves in one sentence — the escalation rows, this table, the README, `self-test.json`'s new `oq2_decided_by` and `oq2_answered_by_brief_author: false`, and every report the tools print. Two determinism-suite checks now assert the distinction so it cannot erode. **(e) Two things are NOT decided and say so.** [B]'s *"confirm the invocation with your mentor"* is an instruction to ask a person; it did not happen and is now a named README limit. And the sweep-legibility metric is **split**: [B]'s acceptance criterion 3 is **met on measured evidence** and names no method, while the PRD's own mentor-review method was **not executed** — recorded as such rather than rewritten to a method this project already passes. **(f) The rev-1 instruction that the implementer must not close OQ-2 alone stands and was honoured**: the implementer did not close it; the DRI did. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 7 | 2026-09-01 | **M7 delivered, and the honest part of it is what it does NOT claim.** **(a) CR-DOC-1 is met**: the README carries [B]'s four topics, each as its own section titled in [B]'s own words and indexed from the top, with the limits section carrying everything the PRD requires plus what M5 and M6 measured afterwards. The CLI stays documented by four golden `--help` files and four builds that fail on a drift. **(b) CR-DOC-2 is met in three of its four parts, and the fourth is reported outstanding.** The committed campaign ships its configuration, its whole report and a `MANIFEST.md` of each capture's size, SHA-256 and read-back counts — **not its 527 MB of captures**, which `.gitignore` has excluded since this repository's first commit (F-34; the manifest itself states what it buys and what it does not). The determinism notes carry five unexplained observations. **The 5-minute recording is NOT DELIVERED**: it is scripted beat by beat in a runbook, including what not to say, and is reported as outstanding rather than substituted for — because substituting a written walkthrough and calling the requirement met is precisely the failure R10 exists to name. **(c) One success metric is UNMET and says so.** "Sweep legibility" names *mentor review of the sweep report* as its method, and no mentor has reviewed it. The artifact exceeds the target — a non-monotone count peaking at 170–190 m/s, three conditions whose verdicts flip at three thresholds, and a run outcome that flips with them — and that is not what the metric measures. Marking it met on the author's own judgement is the move that makes a metrics table worthless. **(d) Both unmet items trace to E-1**, deferred by DRI decision on 2026-09-01, and it was predicted in those words rather than discovering it here. **(e) Every prior milestone's evidence is now committed** — the `.gitignore` change admits any campaign's report, so M2's twenty runs through M6's twenty trace to records rather than to one machine's directory, at 1.2 MB total. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 6 | 2026-09-01 | **OQ-5 is decided, and it was decided by writing M6's conditions in the vendored shape and evaluating them against seven real captures rather than by reading the schema.** **(a) The declaration shape is ADOPTED and one key is ADDED.** Three rules around it are superseded, each on something measured: the unknown-key rule is inverted (F-23's cost, applied to a second author-written file); the verdict becomes three-valued (the vendored `met: false` is a conclusion drawn from absence, which CR-AS-4 forbids); and `scenario_unload` is refused as a `removal_reason` — measured, **267 of 385** removals across the committed sweep, every one at `sim_time_s` 0. The added key is **`expect`**, found by *running* the committed example: without polarity, `command-centre-destroyed` asserts that the command centre **should** be destroyed, the exact inverse of [B]'s own third question. Two of [B]'s three questions survive being read as "this should hold" and the third does not, and the vendored shape expresses non-occurrence only for the `area` kind. Under the PRD's own resolution rule — *"adopt unless a required condition cannot be expressed"* — that is the rule firing. **(b) A gap in `contract/`, and the first that could not be handled by implementing what the text says.** `condition-file-schema.md` is a **verbatim excerpt that stops one heading early**: the sections stating how a distance is computed and how a boundary is decided did not cross, and both are required by CR-AS-2's reproducibility criterion. From the digest alone `within_m` has no stated metric, so two projects could parse the same file and disagree about the same capture, invisibly. Raised as **E-5** ([issue #3](https://github.com/EgeCankaya/EXT-08/issues/3)); F-19 upgrades from *"cannot be verified"* to *"verified by correspondence, and here is what the check found"*. **(c) CR-AS-4 is classified per FORM rather than per kind**, which is finer than the requirement asks: `terminal_state`'s two forms differ completely, and the `removal_reason` form is soundly decidable in the negative by **format §8.1's normative invariant** — which is what makes [B]'s own dangerous example answerable rather than permanently `indeterminate`. **(d) Three defects found by running, none by testing, and one shipped at M4.** **F-31**: every difference in an array-valued field printed two EMPTY values, so `positionGeodetic`, `velocityNed` and `orientationYprRad` named the field and showed nothing; it survived three milestones because reaching that code needs a real content-gate failure on a real pair, and until M6 the gate had never failed on one. **F-26**: the locale-safety suite's own capture builder was locale-dependent. **F-35**: F-24's fix broke again when M6 renamed the outcome a judged run gets, so the committed sweep table printed `-` on every row. **(e) Three new risks. R15** — the axis update races the roster burst and can fail the gate, stopping the **whole campaign** rather than one run (F-29, met on the first execution of the twenty-run campaign, kept at `campaigns/m6-gate-refused/`); **R16** — the continuity bound rests on one inferred assumption (F-28); **R17** — a defect can hide behind a path only a real failure reaches (F-31 generalised). **The gate was not weakened and no retry was added**, and F-29 was the first real opportunity to do either. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 5 | 2026-08-31 | **OQ-4 is decided, and it was decided by exercising a range rather than by arguing from a feasibility result.** **(a) The axis is [B]'s first — initial positions and velocities**, as one declared scalar applied to named entities before `start` (18 probe runs). It won on the two criteria rev 3 named and on one it did not: **fidelity, measured** — the injected value is honoured exactly, first frame to last; **CR-AS-4 soundness** — its effect appears as different values in samples that are *present*, the only one of the three of which that is true; and **roster stability**, 42 authored keys at every value, so results are comparable across the sweep and nothing presses on the comparison's key-mismatch rule. "Which scenario from the catalogue" lost on **ordering**: the platform's own query answers 10 scenarios in 119 ms and a name has no order, so it can vary a result but cannot show a *trend*, which is what [B]'s acceptance criterion 3 asks for in those words. "Which entities are present" lost on CR-AS-4, though less bluntly than rev 3's shorthand suggested — the delete *is* positively recorded as `entity_remove reason: "commanded"`, and it is the axis's *effect* that reads as a zero-sample count. **(b) A new risk, R13 — the axis has a fidelity ceiling the tool does not enforce.** An injected speed is honoured to **400 m/s** and clamped above it at 20 m/s²; at 900 m/s a run spends **42% of itself** off parameter. Found by exceeding the bound. **(c) A new risk, R14, and it is the cost of the chosen axis:** applying a parameter before `start` can land between two publications of the start-up roster burst, making segment 0 `frozen` with repeated values that **differ** — a third shape through §5.1's one test, and the only one this project causes. Measured **4 of 35** parameterised runs (11.4%) against R12's 1 of 27 ordinary ones (3.7%), consistent across two independent batches: elevated, on a sample that supports a direction and not a number, and recorded as elevated rather than as established. **The gate is not weakened and no retry is added.** R12 is extended to carry the parameterised rate. **(d) CR-PAR-1's third criterion is discharged by demonstration**: four runs at one value, all six pairings, **293 576 samples compared, zero differing** — and the negative control holds, two runs 0.1 m/s apart correctly failing the gate with 33 546 differing samples. **(e) CR-PAR-2 is met with its own honesty rules stated**: the sweep is ordered by parameter value, the result column is a count read off each capture rather than a verdict (no condition exists until M6), and a discrete counter near an engagement threshold is genuinely non-monotone — measured, and deterministically so. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 4 | 2026-08-31 | **M4 delivered the determinism gate without the ruling it was gated on, and this revision records what that cost rather than absorbing it.** **(a) OQ-2's decision target of *"before M4 opens"* was MISSED** — the milestone opened, ran and closed with no reply, and that is recorded here rather than quietly re-dated. What M4 did instead of waiting is the substance: **both readings are built as selectable gates** (`--gate-basis content|bytes`, default `content`), both comparisons always run and are always reported, and under `bytes` the gate correctly fails and the campaign correctly stops — exit 3, no campaign run attempted, demonstrated on real runs rather than argued. **A ruling now changes a default and no code.** OQ-2 is re-targeted to *before M7's documentation is written*, because CR-DOC-2 is where an unresolved gate basis would become permanently invisible. **The milestone claims a conditional pass and states its condition; it does not claim [B]'s acceptance criterion 2, anywhere.** **(b) H1 is validated, at 190 pairs rather than the one the signal asked for**: every pair of M2's twenty runs, **9 573 667 samples compared, 9 573 667 agreeing, zero differing, 190 of 190 passing, 0 of 190 byte-identical** — the inherited 50 358 / 50 358 reproduced in kind and exceeded in extent by a factor of 190. **(c) A new risk, R12**, and it is the honest cost of the gate: measured over 42 captures, **2** have a `segment 0` the format's exact frozen-clock test calls `frozen` because part of the start-up roster burst was published twice with byte-identical values — the clock never reset. About **1 pair in 14** on this machine, so a campaign refuses to start that often for a reason that is **not** a determinism failure. **The exclusion is not relaxed and there is deliberately no retry**; the refusal is made to name which shape it found instead. **(d) R11 is re-rated Low → Medium** on a second instance in as many milestones: an imprecision in `contract/` is a rate, not a coincidence, and this one has an operational cost where §6.7's did not. **(e) E-3 and E-4 have been SENT** to EXT-08 as issues, moving them from *recorded* to *raised*; **E-1 has now been drafted-and-unsent across four milestones** and its delivery is blocked on nobody but this project. **(f) A findings index was added** — one index over every issue found, because they were recorded in five places under section names that differ per milestone. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
| 3 | 2026-08-31 | **Two open questions resolved and two risks re-rated, every one of them on this project's own measurements rather than on inherited numbers.** **(a) OQ-6 is decided in both its halves** (M3): a campaign ceiling of **8 GiB over the whole campaign directory** — captures *and* logs, because a capture-only projection under-states a campaign by 22.6% and a pre-flight check that passed on it could still let the disk fill with logs — checked against free space before run 1 and against actual usage after every run, with reaching it stopping the campaign under a named outcome; and **`--on-size-limit stop`** per run, with `--capture-max-bytes` defaulting to `61 000 × --frames`. **The choice was made by exercising rotation on a real run rather than by reading about it**: `rotate` works completely and is not lossy — a four-part capture stitched back to an unrotated run's roster lifecycle exactly — and was declined because one run's two segments become five `(part, segment)` keys, four of them fragments of one segment that nothing in any file identifies as such. CR-CAP-5's ceiling stops being unset. **(b) R9 is closed by measurement** and OQ-4's framing corrected: M2's spike found **all three** of [B]'s axes reachable over the bus with no authoring into `C:\N8RO`, so the axis choice at M5 is free on merit rather than forced, and "which scenario from the catalogue" is no longer a *fallback*. The rev-1 rating of "Unknown — not yet investigated" was correct when written and is now false. **(c) H4 is validated**: the reader passes 78 conformance checks over the vendored fixture, six mutations and thirty real captures, links neither EXT-08 nor the SDK, and cost well under a day against the ~2–3 days the entity picture cost upstream. **(d) A new risk, R11**: a specification can be imprecise as well as stale, and §6.7's summing rule is wrong for `segments` — raised with EXT-08 as E-3 and not worked around. **(e) M3's validation line is corrected** to claim only the half of CR-CAP-1 that M3 can meet. No requirement was removed, no quotation changed, and no provenance marker relaxed. |
...
| Determinism self-test, content comparison | n/a | 100% of compared samples agree, across running segments only. **Inherited reference point: 50 358 / 50 358 on the headless host** [C1] | `compare` over two same-configuration captures | M4 — hard gate. **MET at rev 4: 9 573 667 / 9 573 667 over 190 pairs, 0 differing, 190/190 passing** |
| Determinism self-test, byte comparison | n/a | **Expected to FAIL, and that is not a defect.** Recorded and explained, never used as the gate | Same pair of captures | M4. **MET at rev 4: 0 of 190 pairs byte-identical.** Reported on every self-test, and additionally *executable* as a gate so that OQ-2's answer costs no rework |
| Comparison-path variability introduced by us | n/a | Zero: no wall-clock value, no unordered iteration, no locale-dependent formatting anywhere in the compared output | Unit tests on the comparison path, run on every change | M4. **MET at rev 4**, and closed twice over: six build-time searches that **fail the build** on a hit — each verified to reject by injecting its hazard — plus behavioural tests, including the report regenerated byte-identically under `German_Germany.1252` |
| Determinism self-test refusals that are not determinism failures | n/a | **Not zero, and measured rather than assumed.** ~1 pair in 14 on this machine | The frozen-segment scan over every capture held | M4 — **new at rev 4, see R12** |
| Sweep legibility | n/a | A result that varies with the parameter, presented so the trend is visible without further processing | Mentor review of the sweep report | M5 - **MET at M7, by its own named method.** The mentor reviewed the sweep report on 2026-09-01 and confirmed it reads; relayed by the DRI. **This row was reported UNMET at rev 7 and again at rev 8**, on the grounds that its named method was mentor review and no mentor had reviewed it - the artifact exceeding the target was explicitly NOT accepted as a substitute. The method has now been executed, so the row is met on the terms it was written on rather than on relaxed ones. The artifact: engagement rising 47 -> 65 to a peak at 170-190 m/s and falling to 54 at 380, three conditions flipping at three thresholds, and the run outcome flipping with them. **[B]'s own acceptance criterion 3 names no reviewer and was already met on measured evidence**; that remains true and is a separate statement |
...
- **H1 — The gate holds on content.** We believe **keying the determinism self-test on per-`(entity, occupancy)` value sequences aligned on `sim_time_s`, excluding frozen-clock segments**, will pass repeatably on the headless host, because the property the gate protects — that the same scenario produces the same simulation — was measured to hold exactly while the byte-level representation did not.
  ...
  ***Validated at M4, and by a margin the signal did not ask for.*** *"Repeatedly, not once" is met at **190 pairs**: every pair of M2's twenty runs of one configuration, compared by this project's own binary — **9 573 667 samples compared, 9 573 667 agreeing, zero differing, 190 of 190 pairs passing**, and **0 of 190 byte-identical**. The inherited reference point was 50 358 / 50 358 on a single pair [C1]; this reproduces it in kind and exceeds it in extent by a factor of 190. **H1 holds.*** *Two qualifications, both stated rather than absorbed: the gate additionally **refuses** on a small measured fraction of pairs for a reason that is not a determinism failure (R12), and the hypothesis is about **one machine** — ADR-1's untested half is untouched.*
...
*Which of the three axes [B] offers was OQ-4. **Resolved at M5: initial positions and velocities**, as one declared scalar applied to named entities before `start`. Decided by measurement rather than by argument — 18 probe runs — on fidelity, on what CR-AS-4 can soundly assert, and on ordering, which is what a **trend** requires and a scenario name cannot have. R9's premise, that the choice might be forced by the read-only install tree, did not hold for any of the three and was closed at M2.*
...
**⚠ NAMED DEVIATION (deliverable 3, second clause).** [B] asks for *"its captured runs"* and **the captures are not committed.** One 1200-frame run writes ~24 MB of JSON Lines, so the twenty-run campaign alone is 527 MB; `.gitignore` has excluded captures since this repository's first commit. What ships instead is a `MANIFEST.md` per campaign giving every capture's byte size, SHA-256 and this project's own reader's counts of it, plus everything a **judgement** rests on — `run.json`, `verdicts.jsonl`, `campaign.json`, `campaign.log`, `self-test.json`. **What the substitute buys is narrower than the clause and is stated rather than implied**: it does not make a re-run byte-identical — nothing does, and that is this project's central measurement — but it pins what these specific files contained, so every number in the report traces to a file rather than to somebody's memory, and the counts are what a re-run can be compared against. The deviation is this project's decision, taken on a size measurement, and it is **not** escalated: [B]'s clause is not ambiguous and no ruling would change the disk arithmetic.

**Acceptance criteria:**
- The committed campaign is the 20-run campaign of CR-EX-7, re-runnable from its committed configuration.
- **The captures are not committed and a `MANIFEST.md` stands in for them, in every campaign directory, covering every uncommitted capture** — not only the campaigns of the milestone that wrote the generator. The five spike directories under `campaigns/` are not campaigns and carry none.
...
- **The recording needs a person and a screen recorder.** It is called out here rather than assumed, because the equivalent deliverable was **not delivered** upstream [C2]. Everything it should show is scriptable; the README names the command for each beat. **Met at rev 12: [one take, 4:10, published 2026-09-01](https://drive.google.com/drive/folders/16cR82ynxrcmrzJofwHKdpReNlPj1C--M?usp=sharing)**, shot to a prepared runbook — 4:10 against "5-minute", stated rather than rounded up.
...
| `contract/` — the frozen `n8ro-capture/1` spec and fixture | EXT-08 (vendored, read-only) | **Available and current** — re-pinned at rev 10 to `ca5118c`, the fourth pin, which carries EXT-08's fixes for all four defects raised from here. See R4 and F-38 | A drifted pin costs reader rework; a specification defect must go back to EXT-08 |
...
| A ruling on the determinism gate's basis (OQ-2) | The owner of [B] | **Open — escalated, decided by the DRI, and concurred with by the mentor. Never answered by the recipient** | Did not block in the end: both bases are built, both always run, and the gate basis is selectable. **Criterion 2 is the recipient's to discharge**, so the concurrence raises confidence and closes nothing. A ruling still changes a default and no code |
| Mentor confirmation of the headless invocation (OQ-3) | Mentor | **ANSWERED IN FULL and CLOSED, 2026-09-01** - all six parts across two relays; (c) answered as *"pick the one you prefer"*, so no variant is prescribed | Did not block, and did not change anything: every confirmed answer matched what was already built. (b) additionally makes F-17 firm enough to raise upstream as E-6 |
| A scenario with a varyable axis that does not require writing into `C:\N8RO` (OQ-4) | Mentor / scenario catalogue | **Available — three of them**, measured at M2 | No longer blocks M5. R9 closed at rev 3 |
...
| **R4 — `contract/` is a pinned copy and drifts, and at rev 10 it gained a second cause.** Re-pinned at rev 2 to EXT-08 `0fe7cd5`, producer **0.9.0**, and again at **rev 10 to `ca5118c`** — the fourth pin. The first three drifts were the producer shipping an added key: 0.8.0's `header.sample_form`, then 0.9.0's `header.limits` / `part` / `continues_from` / `continued_in`. **The fourth was caused by this project's own escalations being fixed** (F-38), which nothing watches for. **The vendored fixture still reports producer 0.5.0** and carries none of those keys [C3] | Low — the spec is current again, and every producer drift so far has been an added key that §13 requires a reader to ignore, so a reader built to CR-CAP-2 is unaffected by construction. What a stale pin actually costs is *interpretation*: a reader meets a key its copy of the spec does not describe. The rev-10 drift cost nothing at all, because all four corrections were clarifications | **Re-pinned at rev 2 and rev 10; the fixture's own staleness is unresolved and deliberate** | Re-check the pin at the start of every milestone — twice in two releases is the base rate, not bad luck. **Test against a capture written by the pinned producer, not only against the vendored fixture**, or the newest header keys are never exercised: the fixture predates all of them. CR-CAP-2's ignore-unknown-keys criterion is what makes the drift survivable, and it is worth a test that feeds a header carrying a key the reader has never heard of |
...
| **R9 — The parameterisation axis may require writing into a read-only tree.** [B] offers three axes: initial positions and velocities, which entities are present, which scenario | Low — **closed by measurement.** The premise does not hold for any of the three axes | Low | **Closed at M2 by the `tools/spike-axis` probes**: all three of [B]'s axes are reachable over the bus with **no authoring into `C:\N8RO`**. Entity state set between `load_scenario` and `start` survives materialisation and is integrated from; `sendEntityDelete` and `sendEntityCreate` both take effect; a second scenario loads from the catalogue by name. The rev-1 rating of *"Unknown — not yet investigated"* was correct when written and is now false. **Two limits stand and are carried into OQ-4**: the spike measured *feasibility, not fidelity* — a reachable parameter value can still produce a scenario that makes no sense — and an entity deleted before `start` still appears in `entity_add` and in `trailer.counts`, so an axis of "which entities are present" must count presence by samples, not by adds |
| ~~**R10 — The 5-minute recording needs a person.**~~ **CLOSED at rev 12.** [B] requires it as a deliverable | Was: low technically, but an undelivered acceptance item | Medium — **the equivalent deliverable was not delivered upstream** [C2] | Scheduled explicitly at M7 rather than assumed, with every step scripted so the recording is a capture of a working command sequence rather than a performance (CR-DOC-2). **Shot and published 2026-09-01** — one take, 4:10. The containment worked in the direction that matters: the deliverable was reported outstanding for five revisions rather than substituted for, and then made |
| **R11 — `contract/` can be imprecise as well as stale, and an imprecision is harder to notice than a drift.** Found at M3: §6.7 says a rotated run's totals are the sum across its parts' `counts`; for `segments` that is false, because a segment cut by a rotation is closed in one part and opened in the next. Measured — a real four-part capture summed to 5 for a 2-segment run | Low — it bites only on rotated captures, which OQ-6's `stop` means this project does not produce; and it is arithmetic, not data loss | Low — the reader implements what §6.7 says, computes what is true beside it, prints both and names the gap | **E-3, FIXED by EXT-08 on 2026-09-01** ([issue #1](https://github.com/EgeCankaya/EXT-08/issues/1)) and vendored back at the fourth pin. Never worked around and never propagated. R4's mitigation — re-check the pin every milestone — catches a *stale* copy; only reading the specification against a real file catches an *imprecise* one, which is the argument for the conformance suite's fourth tier existing at all. **Rev 4: a second instance found at M4 and rated Medium** — §5.1's frozen-clock test is said to detect a reset clock and detects two phenomena (E-4, [issue #2](https://github.com/EgeCankaya/EXT-08/issues/2), **also fixed and vendored**, and fixed *wider* than it was raised: EXT-08 folded in R14's third shape too). Two instances in two milestones is a rate, not a coincidence, and the second one has an operational cost where the first did not — see R12. **Rev 10: all four instances of this risk are now fixed upstream, and the risk that replaces it is F-38** — a fixed escalation makes `contract/` stale and nothing notices |
| **R12 — A campaign can stop at its own determinism gate for a reason that is not a determinism failure, and on this machine it does.** Measured at M4 over 42 captures: **2** have a `segment 0` that the format's exact frozen-clock test classifies `frozen` — part of the start-up roster burst published twice with byte-identical values, inside a segment with 1 200 distinct `sim_time_s` spanning 0 to 60. The clock never reset. **~3.7% of ordinary runs, so ~1 pair in 14** | Medium — a campaign refuses to start roughly one attempt in fourteen. It never produces a *wrong* answer: the refusal is named and no run is attempted. The cost is an operator interruption on an unattended campaign, which is exactly the thing G1 exists to avoid | **Measured, not estimated** — 1 of 27 ordinary unmanipulated full runs across M2, M3 and M4 | **The exclusion is not relaxed and there is deliberately NO retry** — a harness that re-rolls its gate until it likes the answer has no gate, and §14 says to exclude frozen segments in those words. Contained three ways instead: the refusal *names the shape it found* (a duplicated publication or a reset clock, distinguished by whether the repeated records' values agree), which is CR-DET-3's requirement; the frequency is in the README's limits section so an operator expects it; and the imprecision went to EXT-08 as **E-4**. **If the rate is found to be materially higher at M5's or M6's scale, that is an escalation about the host, not a reason to weaken the gate.** **Rev 5:** M5 measured it at sweep scale for *parameterised* runs and found a higher rate on a small sample, with a new refusal shape — that is **R14**, and this row's ordinary-run rate is unchanged |
| **R13 — The chosen axis has a range the platform enforces, and nothing at run time checks a value against it.** Measured at M5 over seven values: an injected speed is honoured exactly from the run's first frame to its last up to **400 m/s**, and above it the platform decelerates the entity at exactly 20 m/s² until it reaches the ceiling. At 440 m/s a 60 s run spends 2.0 s off parameter; at 900 m/s, **25.0 s — 42% of the run** | Medium — a sweep crossing the ceiling produces runs that are valid, deterministic, correctly reported and **about a different parameter than the one plotted**. It fails silently, which is the shape that matters | **Measured, not estimated** — by exceeding the bound rather than by assuming one. The ceiling belongs to this entity profile; another scenario's may differ and nothing here establishes where | The committed example stops at 220 m/s and says why in the file itself; the range and the mechanism are in the README's limits section. **Deliberately not enforced in code:** the ceiling is a property of a scenario's entity profiles, not of the campaign runner, and a hard-coded 400 would be this project asserting something about scenarios it has never loaded. If a second scenario is ever swept, its ceiling is measured first |
| **R14 — Applying a parameter before `start` raises the rate at which the determinism gate refuses, and produces a refusal shape that is new.** The axis acts between `load_scenario` reporting loaded and `start` being published, and the platform sometimes republishes part of the start-up roster burst. When the update lands between the two publications the segment carries an instant twice with **differing** values — 31 duplicated instants in such a run, 12 identical (untouched entities) and **19 differing** (updated raiders) — so segment 0 classifies `frozen` and the pair is refused. Measured **4 of 35** parameterised runs, 11.4% against 3.7%, across two independent batches | Medium — the same operational cost as R12, at a rate that looks higher. It never produces a *wrong* answer: the refusal is named and no run is attempted | **Measured, and reported as elevated rather than established** — 4 of 35 against R12's 1 of 27 is a small sample and is stated as one; it is three times the rate, consistently, on numbers that support a direction rather than a value. What *is* established is the shape, which is a **third** phenomenon through §5.1's single test | **The exclusion is not relaxed and there is deliberately no retry**, exactly as R12. Contained the same three ways: the refusal names the shape, the rate is in the README's limits section, and it strengthens **E-4** rather than opening a new escalation — §5.1 presenting one test as detecting one phenomenon is the same finding, now with three. **If a full sweep's rate is materially higher, that is an escalation about the host, not a reason to weaken the gate.** A mitigation exists and was NOT taken: applying the parameter after `start` at frame ≥ 1 avoids the roster burst entirely (M2's `p2b` measured a mid-run update taking effect on the next frame and persisting), but it makes the axis "state at frame 1" rather than an initial condition, and [B]'s axis is *"initial positions and velocities"* |

| **R15 — The axis update races the start-up roster burst, and when it loses, the whole campaign stops rather than one run.** R14's mechanism with a larger blast radius. The axis acts between `loaded` and `start`; the burst is published at load. When it is republished after the update in **one** run of the self-test pair and not the other, one capture carries the scenario's authored velocity and the other carries ours — so the two differ, the gate correctly fails, and **no campaign run is attempted at all**. Measured on the first execution of the committed twenty-run campaign: **23 samples differing, every one at `sim_time_s` 0, every one in `velocityNed`, every one a raider the axis updates** | **Medium-high** — R14 costs one run of twenty; this costs all twenty, and an unattended overnight campaign returns nothing. It never produces a *wrong* answer: the gate refuses, names the shape, and stops | **Measured once, on a real twenty-run attempt**, and the sample is one. R14's 11.4% per-run rate applies to each of the two self-test runs, so a pair is exposed roughly twice per campaign attempt — that is arithmetic over a measured rate rather than a second measurement, and is stated as such | **The gate is not weakened and no retry is added**, and this was the first real opportunity to do either. Contained the same three ways as R12 and R14: the refusal names the shape, the rate and the consequence are in the README's limits section, and it **strengthens E-4** rather than opening a new escalation. The refused execution is **kept** as `campaigns/m6-gate-refused/` and counted, rather than discarded in favour of the clean one — M5 §5's rule applied to itself. **What makes it visible is F-30**, a ~1e-14 platform artifact in an injected `velocityNed` where the authored value is exactly 0; with a bit-identical vector the race would produce no difference at all. If the rate proves material at larger scale, that is an escalation about the host |
| **R16 — The soundness of a not-met geometric verdict rests on one assumption that is inferred rather than measured.** CR-AS-4's continuity bound is `(v_a + v_b) · Δt_max + ½ · a · Δt_max²`, and `a` is F-21's measured 20 m/s² platform clamp — measured on **one entity profile in one scenario** | Low — its practical weight is small: across the committed sweep the acceleration term contributes 0.10 m to bounds of 1.20–22.10 m, and the tightest not-met margin clears its bound by a factor of 64. A wrong `a` would have to be wrong by two orders of magnitude to change a verdict | Low, and **stated rather than discovered** — named in `src/assert/Judge.h`, and in the README's limits section as the assumption to attack first | The term exists to stop the bound being exactly zero for two static entities, which is a real case rather than a hypothetical one. **The alternative was worse**: calling every not-met verdict `indeterminate` is the literal reading of "absence is not evidence" and it makes the tool useless, including for the one condition the committed sweep exists to show. If a second scenario is ever swept, its clamp is measured first — exactly as R13 requires for its ceiling |
| **R17 — A defect can hide behind a path that only a real failure reaches.** Found at M6 as **F-31**: every difference in an array-valued field printed two EMPTY values, so `positionGeodetic`, `velocityNed` and `orientationYprRad` named the field correctly and showed nothing. It shipped at M4 and survived three milestones | Medium — it is a *reporting* defect, so nothing computed from it was wrong; what it cost was CR-DET-3's and CR-REP-4's central promise, that a failure names the deciding **values**. An operator meeting a real divergence would have been told where and not what | **Certain in hindsight, and the mechanism generalises.** Two independent things had to be false at once: the content gate had never failed on a real pair (M4's failing-gate evidence came from forcing `--gate-basis bytes`, which reports a byte offset and never reaches that code), and every synthetic capture in the suite carried only scalar fields | **Both halves fixed** — a verbatim renderer, and an array field in the determinism suite's own capture builder with a check asserting on both values. **The generalisable mitigation is the one worth keeping**: a code path that only a real failure reaches needs a test that manufactures the failure, and *"we have never seen this fail"* is a reason to write that test rather than a reason not to. The same reasoning found F-26, where a suite written to prove locale-independence was itself locale-dependent |

### Open questions

> **For a reviewer.** The rows below that read *decided* or *resolved* were settled inside this
> project rather than by the person the question was addressed to, because delivery time was the
> binding constraint. The README's [Decisions taken without a
...
| **OQ-2** | **Is the determinism gate keyed on content, or on bytes?** | **Decided at M7 and CONCURRED with - content. STILL NOT answered.** | Was: *before M7's documentation is written* | **DECIDED by the DRI on 2026-09-01, from [B]'s own words; CONCURRED with by the mentor the same day, independently and without having seen the reading; and never ANSWERED by [B]'s author, who has not replied.** Those are three different words, and every artifact carrying the decision says all three. **The concurrence is a second opinion and not a ruling** - OQ-2 asks whether [B]'s acceptance criterion 2 is discharged, [B]'s acceptance criteria belong to [B]'s author, and nobody else can transfer that. `self-test.json` keeps `oq2_decided_by`, `oq2_concurred_by` and `oq2_answered_by_brief_author` (`false`) as three separate keys so a consumer cannot compute the last from the others, and two paired tests assert the concurrence never appears without the caveat. **The deciding passage is not criterion 2's "identical captures" but [B]'s statement of what the self-test is FOR** - *"if it ever fails, you have found either a defect in your harness or something far more interesting, and you must be able to tell which."* A byte gate fails 190 of 190 here, identically on a clean harness and a broken one, so it distinguishes neither case and defeats the purpose [B] states for the very test criterion 2 is about; the content gate passed 190 of 190 clean **and failed on a real pair for a real reason** at M6 (`campaigns/m6-gate-refused/`), which is [B]'s harness-defect case caught and named. Four further passages read for content - *"the same outputs"*, *"show they match"*, *"a timestamp in the compared output"*, *"your comparison path"* - and the words **"byte" and "hash" appear nowhere in [B]**. **No code changed**: content was already the default. What moved is the deviation's status, from named-and-unruled to named-and-ruled-by-the-DRI. Criterion 2 is now claimed as met **under the content reading**, said wherever it is claimed, and is not claimed under a byte reading. **The rev-1 instruction that the implementer must not close this alone stands and was honoured: the implementer did not close it - the DRI did, and authorised it explicitly.** A ruling from [B]'s author would still be acted on |
| **OQ-3** | Is `n8ro-sim-app.exe --sim-config SimEngineHost_SharedMemory --model-path <dir> --schema-file <name>` the intended production invocation? | **ANSWERED IN FULL and CLOSED, 2026-09-01 - all six parts, across two relays.** | M2 | **The mentor confirmed all six parts**, relayed by the DRI on 2026-09-01 and recorded here as answers rather than as decisions. [B]'s *"confirm the invocation with your mentor"* is **discharged**. **(b) `N8RO_RELEASE` is expected to be set in production** - confirmed, and it makes F-17 firmer: `PROVENANCE.md` finding 6 gives the invocation *without* it, and following that exactly produces a host that refuses every 42-entity load while sitting idle rather than failing. Raised upstream as **E-6**. **(d) The degraded terrain configuration is expected; leave it as is** - confirmed, so this stops being a judgement call and F-14 stops being ours to defend. **(e) A console control event is the intended way to end an unattended host AND the non-zero exit (`0xC000013A`) is expected** - both confirmed, which is what lets CR-EX-5 keep telling *we stopped it* from *it died* without reading the exit code. **(f) `C:\N8RO\bin` on `PATH` is a known second precondition** - confirmed, separate from (b). **(a) the bus-publish route and (c) `SimEngineHost_SharedMemory` over the seven other variants were NOT covered in the first relay** and were held at *not answered* rather than rounded up; **a follow-up closed both later the same day.** **(a) Yes** - confirming the reading taken from [B]'s own first-listed surface. **(c) *"Pick the one you prefer"***, which settles that **no variant is prescribed**: `SimEngineHost_SharedMemory` stays on the brief's own reasoning - *"campaign runs are for the closed configuration"* plus *"determinism first"*, against a transport permitted to drop - and stays as **this project's own choice, explicitly delegated rather than inferred**. What the answer removes is the risk of having chosen against an intent that existed and had not been written down. **No code changed by any of this** - all six answers confirm what was already built and measured |
| **OQ-4** | Which of [B]'s three parameterisation axes does v1 use? | **Resolved at M5** | M5 | **Decided: initial positions and velocities**, as one declared scalar applied to named entities before `start`. Decided by measurement, not by argument: `tools/spike-oq4/`, 18 runs. **Rev 3 freed the choice** (R9 closed — all three axes reachable with no authoring) and named two deciding criteria; rev 5 answers both and adds a third. **Fidelity**, the criterion M2's spike deliberately did not measure: the injected value is honoured exactly, first frame to last, with a measured ceiling at 400 m/s above which the platform clamps at 20 m/s² — found by exceeding it. **CR-AS-4 soundness**: the axis's effect is different values in samples that are present, never an absence; it is the only one of the three of which that is true. And **ordering**, which turned out to decide it against "which scenario from the catalogue" — that axis enumerates correctly through the platform's own query (10 scenarios, 119 ms, asynchronous, so CR-PAR-1's fourth criterion is satisfiable) but a name has no order, and CR-PAR-2 and [B]'s acceptance criterion 3 both ask for a **trend**. The chosen axis carries a named cost, R14. |
| **OQ-5** | Adopt EXT-08's condition-file schema, or supersede it? | **Resolved at M6** | M6 | **Decided: adopt the declaration shape, supersede three rules around it, add exactly one key, and record two limits in the README.** Decided by the method this row prescribed — writing M6's conditions in the vendored shape and evaluating them against the seven committed captures — rather than by reading the schema. **The anticipated reason to supersede did not arise**: the campaign needs no cross-run condition, because the sweep's comparison belongs to the report and not to the condition language. **What the trying found instead** was that the shape holds completely and the material *around* it does not: the unknown-key rule is wrong for a file a person writes (F-23's cost again), the verdict is two-valued where CR-AS-4 needs three, `scenario_unload` as a `removal_reason` is met by every entity in every run (267 of 385 removals, all at `sim_time_s` 0), and — the one that forced a key — **[B]'s third question is an assertion of non-occurrence and the vendored shape expresses one only for the `area` kind**. `expect` is added; the verdict still records the *fact* in the vendored schema's own terms, so EXT-08's verdicts and EXT-17's re-judgements stay directly comparable, which ADR-5 names as a benefit of adopting. **And the check found a gap in `contract/` that had nothing to do with the question**: the digest is a verbatim excerpt that stops one heading before the arithmetic every geometric verdict rests on — **E-5**. The original wording (below) stands as written |
| | *(OQ-5, as written at rev 1)* | | | EXT-08's OQ-6 resolved that EXT-17 *may* adopt or supersede it [C1] [C2] — a permission, not an instruction, and nothing in [B] mentions a condition-file format at all. Adopting it buys a documented shape and a worked example for free, and makes EXT-08's live verdicts directly comparable with EXT-17's re-judgements. Superseding it may be necessary if the campaign needs cross-run conditions, which EXT-08's per-run schema does not express. **Resolved by CR-AS-3's implementation**: adopt unless a required condition cannot be expressed, and record which in the README |
| **OQ-6** | What is the campaign's disk ceiling, what does it do on reaching it, and which per-run bound do we configure upstream? | **Resolved at M3** | M3 | **Three answers, all measured.** (1) **8 GiB, over the whole campaign directory**, not over its captures: a 1200-frame run costs 29 788 003 bytes, of which the capture is 24 297 928 and the host logs 5 490 075, so a capture-only projection under-states a campaign by 22.6% — cross-checked against M2's twenty runs at 22.5%. At [B]'s scale a 200 s run is ~99 MB and twenty are ~1.99 GB, against the rev-1 inherited estimate of ~1.3 GB. (2) **Reaching it stops the campaign with a named outcome**, checked before run 1 against free space and after every run against actual usage — both halves verified against real runs, with completed runs still reading back conformant afterwards. (3) **`stop`**, with `--capture-max-bytes` defaulting to `61 000 × --frames`, three times the measured per-frame capture cost, so every run carries a bound and no normal run reaches it. Rev 2's argument survives contact with the measurement: **a campaign that needs the tail of a run has mis-set its stop predicate (OQ-1)** — the predicate is a frame budget, and a capture reaching its bound before frame N means the bound is wrong, not the run. **Rotation was exercised rather than declined on the documentation**: a real four-part capture, stitched and read back conformantly, reproducing an unrotated run's roster lifecycle exactly. It was rejected because one run's two segments become five `(part, segment)` keys, a cut duplicates its boundary `sim_time_s`, and `counts.segments` cannot be summed (R11) — costs **CR-CAP-4's** per-segment scoping would pay on every run, whether or not any run ever rotates. Rev 2 named "one file per run keeps CR-CAP-4's segment handling simple" as an argument; M3 measured what that sentence is worth |
...
> **Rev 4 — what M4 delivered, and the one thing it does not claim.** **OQ-2 was not answered before the milestone opened and is still unanswered.** M4 did not wait and did not assume the deviation had been granted: it built both readings as selectable gates (`--gate-basis content|bytes`, default `content`), so that **a ruling either way changes a default and no code**. Under `bytes` the gate correctly fails on this platform and the campaign correctly stops — exit 3, no campaign run attempted — which has been demonstrated on real runs, not argued. Both comparisons always run and are always reported whichever decides.
>
> So the milestone's status is **conditionally passed, with the condition named**, and it makes exactly three claims: (1) *deliverable under either answer* — two runs of one configuration agree on every sample present in both at the same simulation instant, and their captures are never byte-identical; (2) *deliverable with the deviation named* — CR-DET-1's content gate passes; (3) **not claimed anywhere** — that [B]'s acceptance criterion 2 has been discharged. Every report this project produces states (3) in words and machine-readably.
>
> CR-DET-1's `samples_not_recorded` exclusion had never fired in any run this project had made; M4 made it fire on a real capture (`--queue-size 4`, 2 755 samples not recorded) rather than shipping it untested.

### M5 — One parameterisation axis, and a sweep (est. 1.5 days)
[B] step 5. **OQ-4 decided by measurement** — initial positions and velocities, as one declared scalar. Declared in a campaign configuration file, swept across a campaign, and presented so the trend is visible in the report's own format.
**Validation:** CR-PAR-1, CR-PAR-2. A sweep in which at least one condition actually changes outcome across the range.

> **What M5 can claim, and what it cannot.** It can claim the axis is declared outside the code, that each run's value is in its record and in the report, that the sweep is ordered by value, and that the range produces a result which changes — CIWS engagement is absent below about 110 m/s and present above it, with closest approach falling monotonically 7 948 → 17 m across the committed range.
>
> It cannot claim a **verdict** changes across the sweep, because no condition exists until M6. CR-PAR-2's third criterion says *"at least one condition in the committed example campaign actually changes outcome"*; what M5 delivers is a per-run **result** — counts read off each capture — that changes, and the seam M6's conditions attach to. **That is the criterion half-met, and it is recorded as half-met rather than as met.**
>
> It cannot claim determinism for the sweep. The gate runs at one declared value and establishes determinism **for that value** (§CR-DET-1, and R14 for what a parameterised run costs the gate).
...
> **What M6 can claim, and what it cannot.** It can claim that conditions are declared outside
> the code and refused by five distinct named errors before any host starts; that every declared
> condition yields exactly one verdict per run, carrying the entities with their occupancies, the
> segment, the deciding `sim_time_s`, the deciding values and — for a sound not-met — the bound
> that made it sound; that `indeterminate` is a verdict state and never a fifth run outcome; that
> a re-judgement of a stored capture is byte-identical to the live run's verdicts, structurally
> and then checked; that all four ugly realities were injected and the campaign continued past
> each, leaving no host running and no partial record; and that **CR-PAR-2's third criterion is
> now fully met** — a *verdict* changes across the committed sweep, where M5 could only show a
> *result* changing.
>
> It cannot claim that a pass proves a run correct. A pass is a statement about the questions
> somebody thought to ask, over the data that reached the file, and the README says so.
>
> It cannot claim that every not-met verdict is a record. A geometric one is a **bounded
> conclusion** whose bound rests on R16's inferred assumption, and every such verdict carries the
> margin and the bound so the claim is checkable rather than trusted.
>
> **And it cannot claim the campaign always runs.** R15 was met on the first attempt at the
> twenty-run campaign: the gate correctly refused and zero runs were attempted. That execution is
...
**Validation:** CR-DOC-1, CR-DOC-2, and every success metric. **The recording needs a person and is scheduled here explicitly, because the equivalent deliverable was not delivered upstream** (R10). **All three validated as of rev 12.**

> **M7 now closes on everything, and the interesting part is that it did not when it was
> written.** Its validation line says *"CR-DOC-1, CR-DOC-2, and every success metric"*, and two
> of those could not be met by this project alone. **Both were reported UNMET at revs 7 and 8
> rather than claimed**, and both have since been met — by their own methods, not by relaxed ones:
>
> - **"Sweep legibility"** names *mentor review of the sweep report* as its measurement method.
>   For two revisions no mentor had reviewed it and this row read **UNMET** — a metric whose
>   named method did not happen is not a metric that passed, and substituting a different method
>   would have been re-writing the target to fit the result. **The mentor reviewed it on
>   2026-09-01 and confirmed it reads**, so it is now met *on the terms it was written on*. The
>   original refusal is kept on the record beside it, because it is what makes the pass mean
>   anything.
> - **The 5-minute recording** needed a person (R10). M7 delivered the runbook it would follow —
>   so the remaining work was the recording and not the preparation — and reported the deliverable
>   as **outstanding** rather than substituting a written walkthrough and calling it met, because
>   that substitution is precisely the failure R10 exists to name. **[Shot and published
>   2026-09-01](https://drive.google.com/drive/folders/16cR82ynxrcmrzJofwHKdpReNlPj1C--M?usp=sharing): one
>   take, 4 min 10 s, every step of the runbook on camera in order.** The campaign it launches is
>   committed at `campaigns/demo/`. It ran **4:10 against "5-minute"**, stated rather than rounded
>   up. R10 closes.
...
| Deliverables | 5 | 5 | CR-DOC-1, CR-DOC-2. **1 with a named deviation** (deliverable 3's *"its captured runs"* — see CR-DOC-2) |
| Stretch goals | 3 | 3 | All deferred with dated Out-of-Scope entries |
| **Total** | **53** | **53** | |

**Fully covered: 51. Covered with a named deviation: 2** — acceptance criterion 2 (see ADR-1, OQ-2) and deliverable 3's *"its captured runs"* (see CR-DOC-2). **Dropped: 0.** *Corrected at rev 13: the second deviation was announced in the README and carried in the rev-7 revision history, and had never reached this accounting or CR-DOC-2's acceptance criteria. A coverage figure that omits a deviation the project itself discloses elsewhere is the failure this table exists to prevent.* One element is covered by *declining* it on stated evidence rather than by implementing it — the scripting-namespace route (Option 5) — and one is covered by *reporting it as a defect in [B]* rather than by building to it (`EntityStateSample.h`, R7).
...
**WHEN** a reviewer runs the committed campaign configuration **into a scratch `--out-dir`** and watches the recording
**THEN** the campaign reproduces, the report matches the committed one in structure, each committed capture's counts are comparable against the `MANIFEST.md` that stands in for it, the determinism notes state what was needed to make comparison meaningful and carry an unexplained-observations section, and the 5-minute recording shows launch, run and report
**AND** the committed campaign is never the `--out-dir`: `campaign.log` is opened `"wb"`, `campaign.json` is rewritten and runs land in `runs/000` upward, so reproducing over it would destroy the evidence being reproduced
