...
**Revision 8 (superseded):** — **OQ-2 and OQ-3 are DECIDED by the DRI, and neither was ANSWERED.** Schedule became binding, so both were decided from [B]'s own words rather than waited on; `docs/m7-oq2-oq3.md` carries the readings. **No code changed by either** — what moved is what this project may *say*: criterion 2's deviation is now **ruled**, by the DRI and not by [B]'s author. A fourth escalation state, `decided`, keeps that apart from `answered` everywhere it appears. Two things remain undone and say so: [B]'s *"confirm the invocation with your mentor"*, and the 5-minute recording. Rev 7 delivered M7; rev 6 resolved OQ-5. No requirement removed. See §"Revision history".
**Status:** Draft
...
> **M7 closes on everything but the recording — and it did not close on that when it was
...
> rather than claimed**, and one has since been met:
>
...
>   anything.
> - **The 5-minute recording** needs a person (R10). **Still outstanding.** M7 delivers the
>   script it would follow — so the remaining work is the recording and not the preparation — and
>   reports the deliverable as outstanding rather than substituting a written walkthrough and
>   calling it met. That substitution is precisely the failure R10 exists to name.
