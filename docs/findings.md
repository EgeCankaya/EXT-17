...
| F-46 | pair test | **Two committed files carry escape-sequence corruption, and one of them makes the flagship build execute a line of its own prose.** `tools\n8ro-campaign\build.cmd` held `rem tools` + a literal **LF** + `8ro-judge` + a literal **BACKSPACE** (0x08) + `uild.cmd` where `rem tools\n8ro-judge\build.cmd` was meant. The `rem` ends at the injected newline, so cmd tries to run the remainder: **every build of `n8ro-campaign` prints `'8ro-judge<BS>uild.cmd' is not recognized as an internal or external command`** — and then exits 0, because 9009 is overwritten by the next successful line. `src\run\RunRecord.h:198` has the same class, a literal **VT** (0x0B) where `<runDir>\verdicts.jsonl` was meant | `closed` — both repaired, and a repo-wide sweep for control characters outside TAB/LF/CR finds nothing else in either repository. **Only the `.cmd` one had an effect, and it is the smaller half of the finding.** The larger half is that the flagship build script has emitted a named error on every single execution since the file was written and it reached a handover pass unread: an exit code was checked and the output was not | `tools/n8ro-campaign/build.cmd`, `src/run/RunRecord.h` |
| F-47 | pair test | **`contract/` was stale in TWO artifacts, and F-38's rule covers neither of the ways it got there.** The documented pin check — the one `contract/PROVENANCE.md` writes out in full — had not been run since the fourth pin. Run at the fifth: `capture-format-v1.md` was `a6974af…` here against `86ecf8e…` on EXT-08's `main`, and the sample capture was `a089d23…` against `a848759…`. **(a)** EXT-08 resolved E-7, E-8 and E-9 — its own escalations, settled **on EXT-17's ruling** — and wrote the answers into the frozen specification; this project had ruled on three questions about the format it vendors and was reading a copy predating its own rulings. **(b)** EXT-08 regenerated its sample capture because its own audit found it was producer 0.5.0 against a 0.9.0 build. Neither is an escalation EXT-17 raised, so **F-38 does not fire on either** | `closed` — re-pinned to EXT-08 `bda3904`, all three artifacts vendored **by identity**, and `PROVENANCE.md` carries the fifth pin and the two new staleness routes. **F-38 is widened rather than restated**: an escalation this project *answers* makes `contract/` stale exactly as one it raises does, because the answer lands upstream in the file this directory copies. `docs/escalations.md` now records inbound escalations too, which it never had | `contract/PROVENANCE.md`, `docs/escalations.md`, E-7/E-8/E-9 |
| F-48 | pair test | **The one clean-room defect from EXT-08's list that EXT-17 could not have found alone** — and both projects' READMEs were literally walked to find it. Nothing else; both orders reached a recorded-and-judged run. F-42, F-43, F-44 and F-45's fixes all held on a cold clone from GitHub with `core.autocrlf=true`: 0 CR bytes in `contract/`, all eight build scripts green through `vswhere` discovery, 469 mandatory checks reproduced exactly on a machine-independent count | `closed` — recorded in `docs/clean-room.md` with the commands and the numbers, failures included. The pair test found **six** new findings (F-46 to F-52 less F-48 itself), of which **two are blocking for an evaluator** and neither was visible from either working directory | `docs/clean-room.md` |
| F-49 | pair test | **On a fresh clone, nothing exercised the reader against a real producer-0.9.0 capture — while the README and `PROVENANCE.md` both named 0.9.0 as the pinned producer.** The only committed capture was the 0.5.0 fixture, which predates `header.sample_form` (0.8.0) and `header.limits`, `header.part`, `header.continues_from` and `trailer.continued_in` (0.9.0). The tier that reads real 0.9.0 captures is tier 4, whose input is the untracked 569 MB under `campaigns\m2-oq1\runs` — so it ran on one machine and was skipped on every other. **The suite printed its own skip line every single time**, and F-44 had already made that skip visible in the check count without anyone reading it as the coverage gap it was | `closed` — EXT-08's regenerated 0.9.0 sample is vendored as a **second** fixture and `capture_reader_test` gains **tier 1b**, 15 mandatory checks that read it on every machine. **Vendored alongside the 0.5.0 file rather than instead of it**: three of tier 1's checks assert `sample_form`, `limits` and `part` are reported *absent* rather than defaulted (§6.3a, §6.6), which a file carrying all three cannot exercise. Replacing would have traded one coverage for the other; the pair is exactly the compatibility question §13 exists for. Golden 469 → 484 | `contract/capture-atacama-air-defense-sample-0.9.0.n8rocap.jsonl`, `tests/capture_reader_test.cpp`, `tests/checks.golden.txt` |
| F-50 | pair test | **§14's host-dependent exclusion list went from one field to three, and the byte comparison still excludes one.** E-7's resolution — which **EXT-17 itself ruled on** — added `header.continues_from` and `trailer.continued_in` beside `platform.model_path`; both embed the run label, which defaults to an ordinal derived from `--out-dir`, so two rotated sets recorded into different output directories differ there. `src/compare/` masks `model_path` only. **The only item in the fifth pin that did not confirm behaviour already built** — the other three did, which is what every previous pin's whole story was | `open` **by decision, with the bound stated rather than buried.** It is unreachable for any capture this project produces: `--on-size-limit` defaults to `stop` (OQ-6), and an unrotated capture omits both keys — tier 1b asserts exactly that on the vendored 0.9.0 fixture. It can only bite a user comparing two rotated sets recorded elsewhere, on a comparison that is expected to fail on this platform anyway and is not the deciding basis. Not fixed same-day because excluding `continued_in` means splitting the byte walk into head/middle/tail and restating `firstDifferingOffset` — a structural change to the one function the determinism claim rests on — and because ADR-1's position is that this comparison masks as close to nothing as the format allows, so widening it is a decision this project makes by measuring, and there is no rotated pair to measure. §14's own mitigation is `--run-label` | `src/compare/Compare.cpp` (`maskModelPath`), `src/compare/Compare.h`, README, both help goldens |
| F-51 | pair test | **`README.md` never said where EXT-08 is — and the whole of order B is blocked on that one line.** A reader starting at EXT-17 builds all four tools, runs the README's first command, and is told `--recorder <path> is required unless --no-recorder`. The README devotes a section and a four-row table to *"The fourth binary is not in this repository"*, names it as **EXT-08's `n8ro-bridge.exe`**, gives a plausible path — and gives no repository, no URL and no clone command. The only GitHub URL in the repo is an issue link buried in `docs/escalations.md` and the PRD's revision history. **Invisible in order A by construction**: if you build EXT-08 first you already have the thing you would have gone looking for | `closed` — the row names the repository, the three build commands and the output path. **This is the finding the pair test exists for**: it is not a defect in either repository read alone, it is a defect in the seam, and only the order that arrives at EXT-17 first can see it | `README.md` §"The fourth binary is not in this repository" |
| F-52 | pair test | **`report` was documented in three places as needing no N8RO install, and it needs one.** README's recorder table, the README's own command block, `docs/m6-assertions.md`, and `n8ro-campaign --help` all say a reviewer re-reading a stored campaign needs no install. Measured on a cold clone with `C:\N8RO\bin` off `PATH`: exit **`0xC0000135`** (`STATUS_DLL_NOT_FOUND`), **no output and no diagnostic** — the exact shape the README elsewhere documents as *"it looks like a crash and is a missing DLL"*. `report` is a command of `n8ro-campaign`, which links `n8ro-sim.lib` and `n8ro-core.lib`, so the binary cannot load however little that command uses it. `n8ro-judge check` and `n8ro-capture read` were run in the same shell and both exited 0 — the claim is true of them and was over-generalised to a third command in a different binary | `closed` — all four sites now say what is true: `report` needs no **host, scenario or recorder**, and does still need `C:\N8RO\bin` on `PATH`. **Found by executing the claim rather than by reading it**, which is the only way this kind is ever found: the sentence is correct about two of the three things it names | `README.md`, `docs/m6-assertions.md`, `tools/n8ro-campaign/main.cpp`, `tools/n8ro-campaign/help.golden.txt` |

**F-46 to F-52 are the cross-repo pair test's, and the two that matter are in the SEAM rather than in
either repository.** F-51 blocks an evaluator who opens EXT-17 first — the README requires a binary from
another repository and never says which one. F-49 means a fresh clone never read a 0.9.0 capture while both
documents claimed 0.9.0 was pinned. Neither is visible from either working directory, and neither is visible
from *one* repository however carefully it is read: order A hides F-51 by construction, and F-47 and F-49
need EXT-08's `main` in the room to see at all. The other four are ordinary packaging defects of the kind
the tier-0/1 pass found. **F-47 is the one worth re-reading in a year**: the discipline that made `contract/`
trustworthy — raise it, do not work around it — is itself what made the directory go stale, twice, by two
routes the rule written to catch it does not cover.
